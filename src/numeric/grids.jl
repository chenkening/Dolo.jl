...
function <:PointGrid(point::Vector{Float64})
    d = length(point)
...
function <:CartesianGrid(min::SVector{d,Float64}, max::SVector{d,Float64}, n::SVector{d,Int64}) where d
...
<:CartesianGrid(min::Vector{Float64},max::Vector{Float64},n::Vector{Int64}) = CartesianGrid(SVector(min...), SVector(max...), SVector(n...))
...
function  <:Union{RandomGrid,RandomGrid{d}}(min::Point{d}, max::Point{d}, n::Int) where d
